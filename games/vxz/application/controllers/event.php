		...
		if ($this->g_user->uid) {
			...
			if ($check_transfered) {
				...
				$this->_init_layout()
					...
					->add_js_include(array('jquery-1.12.3.min', 'event/jquery.colorbox-min', 'jquery.validate.min', 'jquery.metadata', 'jquery.form', 'event/validate2'))
			...
			$this->_init_layout()
				->set_meta("title", "絕代雙驕玩家獨享元寶活動")
				->add_css_link(array('event/reset','event/colorbox'))
				->add_js_include(array('jquery-1.12.3.min', 'event/jquery.colorbox-min', 'jquery.validate.min', 'jquery.metadata', 'jquery.form', 'event/login'))
				->mobile_view();
		}
	}

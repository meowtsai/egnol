...
<form method="get" action="<?=site_url("trade/pepay")?>" class="form-search">
	...
	<div class="control-group">	
		...
		交易結果
		<select name="TRADE_CODE" style="width:75px">
			<option value="">--</option>
			<option value="N" <?=($this->input->get("TRADE_CODE")=='N' ? 'selected="selected"' : '')?>>失敗</option>
			<option value="Y" <?=($this->input->get("TRADE_CODE")=='Y' ? 'selected="selected"' : '')?>>成功</option>
		</select>	
	...
	<div class="control-group">
		...
		<input type="text" name="ORDER_ID" value="<?=$this->input->get("ORDER_ID")?>" class="input-medium" placeholder="訂單號">
		<input type="text" name="SESS_ID" value="<?=$this->input->get("SESS_ID")?>" class="input-medium" placeholder="PEPAY交易代碼">	
	...
	<tbody>
		...
		<tr class="<?=$row->status=='2' ? 'success' : ''?>">
			...
			<td title="帳號: 
				...
				<a href="<?=site_url("trade/pepay?uid={$row->uid}&action=查詢")?>"><i class="icon-search"></i></a>

...
<table class="table table-striped table-bordered">
  ...
  <tbody>
	...
	<tr>
		<td><input type="checkbox" class="cb" id="<?=$row->server_id?>"></td>	
		<td><?=$row->server_id?></td>
		<td>
		...
		<td><?=$row->address?></td>
		<td style="color:<?=$server_status[$row->server_status]["color"]?>"><?=$server_status[$row->server_status]["name"]?></td>
		<td style="color:<?=$enable[($row->is_transaction_active)?1:0]["color"]?>"><?=$enable[($row->is_transaction_active)?1:0]["name"]?></td>
		<td>
			<div class="btn-group">
				<a href="<?=site_url("server/edit/{$row->server_id}?game_id={$this->game_id}")?>" class="btn btn-mini">修改</a>		
					<a class="btn btn-mini dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></a>
					<ul class="dropdown-menu pull-right" role="menu" aria-labelledby="dropdownMenu">
						<li><a href="javascript:;" class="json_post" url="<?=site_url("server/set_new/{$row->server_id}?game_id={$this->game_id}")?>">設為新服</a></li>
						<li><a href="javascript:;" class="json_post" url="<?=site_url("server/set_entry/{$row->server_id}?game_id={$this->game_id}")?>">設為入口</a></li>
						...
						<li class="dropdown-submenu pull-left">
						    ...
						    <ul class="dropdown-menu">
								<li><a href="javascript:;" class="json_post" url="<?=site_url("server/set_tran_status/{$row->server_id}/1?game_id={$this->game_id}")?>">開啟</a></li>
								<li><a href="javascript:;" class="json_post" url="<?=site_url("server/set_tran_status/{$row->server_id}/0?game_id={$this->game_id}")?>">關閉</a></li>
						    </ul>						
						</li>
					</ul>				
			</div>

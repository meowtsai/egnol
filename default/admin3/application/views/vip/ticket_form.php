...
<div style="padding:20px;">
	...
	<form method="post" action="<?=site_url("vip/modify_json")?>" enctype="multipart/form-data">
	...
	<table cellspacing="0" cellpadding="0" style="width:660px;">
		<tr>
			...
			<td>
				<select name="allocate_admin" class="required" style="width:150px;">
					...
					<option value="<?=$row->uid?>" <?=($vip && $vip->allocate_admin_uid==$row->uid ? 'selected="selected"' : '')?>><?=$row->name?> (<?=$row->role_desc?>)</option>
					<? endforeach;?>
				</select>	
			</td>
			<td>通知其他人員：</td>
			<td>
				<select name="cc_admin" style="width:150px;">
					<option value="">--請選擇--</option>
					<? foreach($admin_users->result() as $row):?>
					<option value="<?=$row->uid?>" <?=($vip && $vip->cc_admin_uid==$row->uid ? 'selected="selected"' : '')?>><?=$row->name?> (<?=$row->role_desc?>)</option>
					<? endforeach;?>
				</select>	
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<select name="urgency" class="required" style="width:150px;">
					<option value="0" <?=($vip && $vip->urgency=="0" ? 'selected="selected"' : '')?>>普</option>
					<option value="1" <?=($vip && $vip->urgency=="1" ? 'selected="selected"' : '')?>>高</option>
					<option value="2" <?=($vip && $vip->urgency=="2" ? 'selected="selected"' : '')?>>急</option>
				</select>	
			</td>
			<td>工作分類：</td>
			<td>
		...
		<tr>
			<td>標題：</td>
			<td colspan="3"><input type="text" name="title" class="required" maxlength="60" style="width:300px;" value="<?=$vip ? preg_replace('/<br\\s*?\/??>/i','',$vip->title): ''?>"></td>
		</tr>		
		<tr>
			<td style="vertical-align:top">描述：</td>
			<td colspan="3"><textarea rows="8" minlength="5" style="width:100%;" name="content" class="required"><?=$vip ? preg_replace('/<br\\s*?\/??>/i','',$vip->content): ''?></textarea></td>
		</tr>

...
<form method="get" action="<?=site_url("ticket/get_list")?>" class="form-search">
	...
	<div class="control-group">	
		...
		<select name="urgency" style="width:90px">
			...
			<? foreach($ticket_urgency as $key => $urgency):?>
	...
	<div class="control-group">
		...
		<input type="text" name="ticket_id" value="<?=$this->input->get("ticket_id")?>" style="width:90px" placeholder="#id">
	...
	<div class="control-group">
        ...
        <select name="admin" style="width:150px;">
            ...
            <option value="<?=$row->uid?>" <?=($this->input->get("ticket_id")==$row->uid ? 'selected="selected"' : '')?>><?=$row->name?> (<?=$row->role_desc?>)</option>
            <? endforeach;?>
        </select>	
        指派人員

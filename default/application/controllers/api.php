...
class Api extends MY_Controller {
	...
	function login_game()
	{
		...
		{
			if (empty($server)) {
				header("location: http://www.long_e.com.tw/play_game/{$game}?url=http://{$game}.long_e.com.tw/index.php?serverin=1");
				exit();
			}
			if (in_array($partner, array('artsy'))) { //使用我方遊戲bar
				header("location: http://www.long_e.com.tw/play_game?sid={$server_row->id}");
				exit();
			}
	...
	{		 
		if ($post = $this->input->post()) {
			...
			unset($post['key']);
			echo "產生網址: http://www.long_e.com.tw/api/create_game_role?".http_build_query($post)."&hash=".$hash;
			echo "</pre>";
		}
		?>
		<form method="post">
...
/*
	...
	function test_check_role_status()
	{
		if ($post = $this->input->post()) {
			...
			echo "加密前字串: ".($post['partner'] . $post['uid'] . $post['game'] . $post['server'] . $post['time'] . $post['key'])."<br>";
			unset($post['key']);
			echo "產生網址: http://www.long_e.com.tw/api/check_role_status?".http_build_query($post)."&hash=".$hash;
			echo "</pre>";
		}
		?>
		<form method="post">
	...
	{	
		...
		if ($euid) { //直接登入			
			...
			header('location: http://www.long_e.com.tw/api/m_login_form?'.http_build_query($output));
			exit();
		}
	...
	function m_open_long_e_billing_v2()
	{
		...
		if ($type == 'ios') {
			$url = "http://www.long_e.com.tw/payment/m_ios_index?sid=".$server_row->id."&partner=".$this->partner."&game=".$this->game;
		}
		else if ($type == 'google') {
			$url = "http://www.long_e.com.tw/payment/m_google_index?sid=".$server_row->id."&partner=".$this->partner."&game=".$this->game."&eg=".$eg."&test=".$operator.$country;
		}
		else if ($type == 'long_e') {
			$url = "http://www.long_e.com.tw/payment/m_long_e_index?sid=".$server_row->id."&partner=".$this->partner."&game=".$this->game."&eg=".$eg."&test=".$operator.$country;
		}
		else {
			$url = "http://www.long_e.com.tw/payment/m_index?sid=".$server_row->id."&partner=".$this->partner."&game=".$this->game."&eg=".$eg."&test=".$operator.$country;
		}
	...
	function m_get_euid() 
	{
		...
		if (get_mobile_os() == 'ios') {
			echo "<script src='http://www.long_e.com.tw/p/js/iosBridge.js'></script>

...
class Rc2 extends MY_Controller {
	...
	function entry()
	{
		...
		if (empty($redirect_url)) $redirect_url = base_url();
	...
	function website_entry()
	{
		...
		$this->_rc2_login_api(base_url()."/gate/website/{$game}");
	}
	
	function service_entry()
	{
		$this->_rc2_login_api(base_url()."/service");
	}
	...
	function payment_entry()
	{
		...
		$this->_rc2_login_api(base_url()."/payment?game={$game}");
	}

...
class Gate extends MY_Controller {
	...
	function login_callback($channel)
	{
		...
		if ($result == false) {
			...
			//直接進遊戲
			if ( ! empty($sid)) {
				header("location: http://www.long_e.com.tw/play_game?sid={$sid}&ad={$ad}");
				exit();
			}
			...
			$url = 'http://www.long_e.com.tw/index.php';
			switch ($site)
			{				
	...
	function check_account_channel($type='')
	{
		...
		else 
		{
			switch ($type) 
			{
				...
				case 'service': header("location: http://www.long_e.com.tw/service"); exit(); break;	
			}
		}
	}
	...
	function _redirect_web($game_id='', $alert_msg='')
	{
		if ($game_id) {
			...
			$redirect_url = "http://www.long_e.com.tw";
		}

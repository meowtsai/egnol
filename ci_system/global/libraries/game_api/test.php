...
{	
	...
	function get_apple_bundle_id()
	{
		...
		return $partner_api['longe']['sites']['test']['Apple']['BundleID'];
	}
...
/*		
		...
		$product_id = "test_" . $amount;
		$price = $amount;
		$currency = "TWD";
		
		$app_key = $partner_api['longe']['sites']['test']['key'];
		$time = time();
...
/*		
        ...
        $res = $this->curl_post($game_api['test']['billing'], array(
													'order_id'=>$order_id,
													'transaction_id'=>$transaction_id,
													'partner_order_id'=>$partner_order_id,
													'product_id'=>$product_id,
